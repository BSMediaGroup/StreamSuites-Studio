...
it("changes and persists all desktop sidebar and header modes from the accessible View menu", () => {
  ...
  expect(JSON.parse(window.localStorage.getItem(STUDIO_PRESENTATION_STORAGE_KEY)!)).toEqual({ sidebar: "expanded", header: "auto-hide", cinematic: "off" });
});

it("delays auto-hide, stays visible while View is open, and reveals from the top activation strip", () => {
  vi.useFakeTimers();
  window.localStorage.setItem(STUDIO_PRESENTATION_STORAGE_KEY, JSON.stringify({ sidebar: "expanded", header: "auto-hide", cinematic: "off" }));
  render(<ThemeProvider><PresentationProvider><MemoryRouter><StudioShell><p>Workspace</p></StudioShell></MemoryRouter></PresentationProvider></ThemeProvider>);
  const shell = screen.getByText("Workspace").closest(".studio-shell")!;
