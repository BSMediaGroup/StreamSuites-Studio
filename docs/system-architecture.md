...
- `PresentationProvider` owns one validated local object for collapsed/expanded/hidden contextual-panel presentation, standard/slim/auto-hide headers, off/on cinematic presentation, and 3/5/8/12-second or manual contextual-notice duration. The first-time panel state is collapsed; the bottom control changes only collapsed/pinned-expanded, while View alone hides/restores it. Those modes change layout classes without remounting the room route or reconnecting SSE. Corrupt fields fall back independently; no room, guest, invite, permission, event, or media value is accepted into that storage object.
...
- The active-room desktop has two independent sidebars. The restored primary Studio sidebar remains on the far left, uses its original header, scrollable navigation, bottom-pinned control, theme/mobile behavior, and a room-specific safe route back to Rooms; the lobby retains the original product destinations. The canonical right contextual panel alone owns Backstage, Invites, Room/custom layouts, Branding, Media, browser sources, and presentation settings. Its default 64px icon rail temporarily expands over the Stage from the right on hover/focus; pinning adds the approximately 360px right grid track, and View-only hiding reserves zero contextual width without affecting the primary sidebar. No internal shortcut-only third rail exists. Contextual tabs use one non-wrapping, non-shrinking horizontal scroller with selected-tab reveal, wheel/touch/keyboard support, a narrow themed scrollbar, and fixed `previous.svg`/`next.svg` edge controls; the production dock retains the same overflow pattern.
