...
- Stage media components keep stable participant keys inside a centered 16:9 output. Exactly two visible participants fill equal full-height columns; usable camera video hides the central fallback while the independently styled/visible name/subtitle overlay remains. Presentation keeps the screen at `object-fit: contain`. Branding backgrounds/logo decorations are pointer-transparent, and safe-area guides are editor-only. Participant actions, delayed icon tooltips, and the Custom layout selector render through viewport-colliding document portals so Stage/panel/dock overflow cannot clip them.
