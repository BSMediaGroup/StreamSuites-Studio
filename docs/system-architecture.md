...
- Runtime/Auth separately owns internal room identity and broadcast title/description, nullable UTC schedule, visibility, opaque thumbnail asset assignment, immutable 1280×720 HTTPS CDN derivative, and safe destination counts. Studio keeps only form drafts and waits for canonical create/update responses; broadcast/thumbnail SSE invalidations refetch the existing room without reconnecting RealtimeKit.
- Destination readiness is status-only: credentials remain server-side, room assignment/output is absent, no private Rumble GraphQL API is used, and Studio remains OFF AIR.
...
- `/studio` presents Runtime room summaries with canonical Room ID chips. `/studio/rooms/:roomId` probes canonical room-management authority before dispatching to director/cohost management or the current guest's safe Stage/Backstage workspace, canonicalizes old UUID URLs, and exposes only permission-summary-approved participant, ordering, presentation, invite, and cohost actions. This keeps signed-in invite participants on their authoritative guest identity even when their account also has general Studio access; deleted rooms stop at the safe not-found surface. Guest joins navigate into this route rather than remaining on a text waiting page.
...
- The active-room desktop shell has three direct tracks: shared left `StudioEdgeSidebar`, uncapped `minmax(0, 1fr)` center workspace, and the mirrored right `StudioEdgeSidebar`. Both instances use the same rail/header/independent-scroll-body/final-toggle DOM contract and full workspace-row height. The right production instance is portaled as a direct shell grid child, not nested under `.production-workspace`, the room header, Stage, dock, Backstage, or a center card. Each first-time 64px rail temporarily reveals its 360px panel from the rail's exact outer edge after resetting the absolute overlay's pinned grid-column placement; only a pinned state reserves the exact 424px rail-plus-panel track, matching 46px square bottom controls only expand/collapse their own edge, and View-only hiding releases only that edge.
- Stage media components keep stable participant keys inside the largest centered 16:9 output allowed by the live center width and remaining shell height after the room header, 58px Stage toolbar, and production dock. The toolbar and output consume the same container-calculated width, stale center maximums no longer cap the output, and pinning either edge recalculates only the available center track while hover leaves the Stage rectangle unchanged. Backstage follows the viewport region in center document overflow instead of shrinking the Stage; both sidebar bodies scroll independently while their rails and final-row toggles stay fixed. Contextual messages remain compact overlays, Presentation keeps `object-fit: contain`, and the shell clips document-level horizontal overflow.
- The bounded 1366×768 local MCP fixture measured identical left/right top, bottom, and height; identical toggle bottoms; 7.99px bottom insets; a 718.61×404.22 collapsed Stage at 1.77777; zero toolbar-edge mismatch; zero hover resize on either edge; and a 691.16×388.77 Stage when either edge was pinned. Scrolling the right body left its rail, toggle, and center scroll position unchanged. No production Runtime/Auth, RealtimeKit, or Cloudflare service participated.
...
- Private room chat uses the same Runtime/Auth participant authority and room SSE stream but is independent from RealtimeKit media. The right sidebar component remains mounted beside the Stage; selecting Chat changes only contextual panel state, so media meeting, participant, and source identities do not remount. Runtime owns history, tombstones, idempotency, unread cursors, moderation, and rate limits.
- Public Chat is status/configuration only. Studio receives a safe registry-derived capability catalog and existing OAuth initiation links where Runtime already supports them; tokens and actor credentials never enter browser state. Provider feed ingestion and outbound transports, including any Rumble write path, are absent.
...
- `src/api/runtimeVersion.ts` validates the existing runtime-owned `version.json` shape. The Studio-local footer ports Public's current bar/links/copyright/meta and `ss-status-*` structure, responsive spacing, status trigger, and version/build tooltip without a runtime source import. It hydrates the configured export and performs one established `/api/health` check per shell mount, with loading/online/degraded states and no aggressive poll. The one Runtime/Auth disclosure uses an opaque theme surface, begins closed, does not participate in layout, and removes its bounded Escape/outside-pointer listeners on close or unmount; pointer leave, blur, Runtime state change, and route change also close it.
...
- Room participant avatars are canonical Runtime/Auth participant fields: uploads wait for the returned versioned CDN URL, refresh and room SSE refetch the same serializer, Stage and Backstage consume the same `StudioGuest.avatarUrl`, and image failure falls back to current initials. A room override never writes the linked account avatar.
- The authenticated topbar Requests inbox lists only the current session account's pending permanent-cohost requests and sends accept/decline without client-supplied account authority. Lobby room edit/delete and invite revoke/delete controls are permission-gated UI over Runtime/Auth endpoints; destructive controls confirm explicitly and update only from canonical success.
