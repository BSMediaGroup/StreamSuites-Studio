...
- The room workspace has one canonical left contextual panel for Backstage, Invites, Room/custom layouts, Branding, and Media. Its default icon rail temporarily expands over the Stage on hover/focus without changing the 64px workspace track; pinning changes that track to the former 360px expanded-panel width. Full hiding reserves zero width and is available only in View. The former desktop right panel, duplicate shortcut-only rail, floating restore control, and three-state bottom cycle no longer exist. The one-row dock scrolls horizontally behind fixed edge arrows instead of wrapping.
