...
- `src/api/runtimeVersion.ts` validates the existing runtime-owned `version.json` shape. The Studio-local footer ports Public's current bar/links/copyright/meta structure, responsive spacing, glass treatment, version/build tooltip, and status disclosure without a runtime source import. It hydrates the configured export and performs one established `/api/health` check per shell mount, with loading/online/degraded states and no aggressive poll.
