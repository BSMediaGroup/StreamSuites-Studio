...
- Stage media components keep stable participant keys and remain mounted while CSS arranges one through nine occupied tiles without open-slot stretching. Presentation uses a large contained 16:9 share plus filmstrip; Interview uses two primary tiles; Spotlight uses the selected primary plus filmstrip. Participant actions render through a viewport-colliding document portal so Stage/panel/dock overflow cannot clip them.
