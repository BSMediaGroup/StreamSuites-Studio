...
- The active-room desktop has two independent full-height edge-sidebar systems around the center workspace. The primary left rail opens dedicated, independently scrolling Studio/Rooms, Brand, Media, Destinations, and Settings panels; every selected product section has useful local content or a truthful Runtime-dependent foundation, and none redirects to the right. The Studio panel uses lobby versus in-room copy/actions and never contains Backstage, invite, cohost, or room-production tools. The mirrored right rail and panel occupy their own room-viewport column spanning room header, Stage, dock, and Backstage rows and exclusively open dedicated Backstage, Invites, Room/custom-layout, Branding, and Media/browser-source content. Each collapsed 64px rail temporarily reveals its adjacent approximately 360px panel as an absolute overlay on hover/focus; pinning reserves the combined rail/panel width, matching square bottom toggles only pin/collapse, and View-only hiding releases only that side. The rejected compressed horizontal top-level room-tab strip and its edge scrollers are absent.
- Stage media components keep stable participant keys inside a centered 16:9 output sized by both the live Stage container width and height. The icon-only Stage toolbar and output share the exact same center-column edges while either sidebar pins, collapses, hides, or peeks. Backstage remains below the Stage inside the center workspace, while the independently scrolling right panel retains full usable height and its bottom control. Exactly two visible participants fill equal full-height columns; usable camera video hides the central fallback while the independently styled/visible name/subtitle overlay remains. Contextual messages are compact absolute overlays with typed tones, manual close, paused timers, and a polite live region; persistent media/connection status remains in the Stage and dock. Presentation keeps the screen at `object-fit: contain`, and the shell clips document-level horizontal overflow.
