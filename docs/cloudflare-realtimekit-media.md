...
## Browser sources are not RealtimeKit media

Runtime/Auth owns custom browser-source records and validates/canonicalizes their URLs without fetching or proxying the page. Studio renders authorized on-Stage sources as sandboxed iframes at a centered normalized rectangle above participant/presentation media and below Branding/controls. Sources begin Backstage, disabled sources instantiate no iframe, limited visible Backstage previews use lazy loading, Stage activation is eager, refresh recreates only the selected iframe, and pointer interaction is off until an authorized operator explicitly enters local interaction mode. Escape exits interaction.

Browser sources never become RealtimeKit participants, add an audio element or `.play()` call, consume one of the nine participant slots, change presentation-source identity, or reconnect the meeting, camera, microphone, screen share, SSE, or participant mappings. The iframe permission policy denies camera, microphone, geolocation, clipboard, payment, USB, Bluetooth, MIDI, display capture, and autoplay. Provider frame policy/authentication can prevent display and is not bypassed. Studio remains OFF AIR.
