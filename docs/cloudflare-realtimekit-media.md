...
`GuestRoomWorkspace` uses the same hook and mapping store. Backstage guests receive Stage audio/video, may run preflight and keep a private local preview, and cannot self-admit. Director-facing Backstage cards remain identity cards and distinguish provider camera readiness from the explicit absence of a scoped director-preview transport; they do not publish ordinary guest camera video or fake it with screenshots/avatars. On-stage guests publish permitted choices and may self-Backstage. Cohost self-stage remains limited to Runtime-granted permission/preset. Requested `auto` remains Runtime-owned while both host and guest workspaces derive the same effective layout; the derivation never mutates provider or canonical room state.
...
The workspace reflow changes only presentation chrome around the media elements. Device preflight, SDK client/token refresh, local/remote audio/video registration, active speaker, provider Stage/Backstage synchronization, screen sharing, and host disable operations remain in the existing hook/elements. The Stage output stays centered at 16:9; two visible participants use two equal full-height columns; the right panel stretches to the complete Stage/status/Backstage workspace; and the dock, panel, tooltip portal, cinematic, and fullscreen changes do not remount stable participant media during ordinary reorder or layout changes.
