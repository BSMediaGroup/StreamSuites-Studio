...
The workspace reflow changes only presentation chrome around the media elements. Device preflight, SDK client/token refresh, local/remote audio/video registration, active speaker, provider Stage/Backstage synchronization, screen sharing, and host disable operations remain in the existing hook/elements. The dock never wraps and the Stage/panel/menu layout does not remount those media components during ordinary reorder or layout changes.
