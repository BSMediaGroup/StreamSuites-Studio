...
- ship collapsed/expanded right contextual-panel presentation plus View-only hiding, standard/slim/auto-hide headers, and local contextual-notice duration through one validated presentation-only preference provider, while preserving the independent primary left mobile drawer
...
- compact the completed media workspace without replacing its lifecycle: header Room Actions replaces the duplicate action row; the original route-specific primary left sidebar and independent collapsed-by-default right contextual panel remain separate; no duplicate shortcut rail exists; right-side hover expansion overlays while pinned expansion resizes; View alone hides/restores the contextual panel; the 16:9 Stage fits the visible room height while Backstage overflows below; contextual notices overlay without reflow; and both the contextual tabs and dock remain horizontally navigable single rows with fixed edge controls
