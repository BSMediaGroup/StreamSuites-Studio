...
- compact the completed media workspace without replacing its lifecycle: header Room Actions replaces the duplicate action row; the route-specific full-height primary left product rail/panel and mirrored full-height right room-production rail/panel remain separate and independently scrollable; every left section retains dedicated local content including Destinations; hover expansion overlays while pinning independently resizes from either edge; square bottom toggles only pin/collapse and View alone hides/restores each system; the compressed horizontal right tab strip is removed; the Stage toolbar and 16:9 output share the center-column edges while Backstage follows below; the right Room panel exposes canonical Fill/Fit; Public footer structure/status/version-tooltip parity is Studio-local; contextual notices overlay without media reflow; and only the production dock retains horizontal overflow controls
