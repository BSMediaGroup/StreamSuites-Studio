...
- protected `/studio/rooms/:roomId` production workspace with canonical short-code URLs, the original primary Studio sidebar on the far left, a viewport-fitted 16:9 Stage/Program canvas, real RealtimeKit media, an independent right contextual Backstage/on-stage/cohost/invite/room/Branding/Media panel, scrolling Backstage below, lifecycle controls, and the existing production dock
...
- two independent desktop sidebars on active-room routes: the restored primary Studio sidebar keeps route-safe Rooms navigation and never duplicates room tabs, while the right contextual room panel owns Backstage, Invites, Room/custom layouts, Branding, Media, browser sources, and presentation settings. The right panel defaults to a collapsed icon rail, temporarily overlays from the right on hover/focus, uses a persisted approximately 360px pinned state, and is hidden only through View; its one-row tab strip scrolls horizontally between fixed themed previous/next buttons instead of shrinking or wrapping. Lobby routes retain the original product/navigation destinations, no third shortcut rail exists, and contextual messages still stack above the Stage without reflow
