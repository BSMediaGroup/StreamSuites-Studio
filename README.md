...
- two independent desktop edge-sidebar systems on active-room routes. The primary left rail opens dedicated, independently scrolling Studio/Rooms, Brand, Media, Destinations, and Settings panels; lobby and room copy/actions remain route-specific. The right vertical rail opens dedicated Backstage, Invites, Room/custom-layout, Branding, and Media/browser-source panels. Either collapsed sidebar reserves only its 64px rail, temporary hover/focus expansion overlays the Stage, and pinned expansion reserves its rail plus approximately 360px panel independently. The rejected compressed horizontal room-tab row is removed; View can hide/restore either system without changing the other, and contextual messages still overlay without Stage reflow
