...
- two independent full-height desktop edge-sidebar systems on active-room routes. The primary left rail opens dedicated, independently scrolling Studio/Rooms, Brand, Media, restored Destinations, and Settings panels with truthful local product foundations and no referral-only panels; lobby and room copy/actions remain route-specific. The mirrored right vertical rail and full-height panel span the room header, Stage, dock, and Backstage rows and open dedicated Backstage, Invites, Room/custom-layout, Branding, and Media/browser-source content. Either collapsed sidebar reserves only its 64px rail, temporary hover/focus expansion overlays the center, and pinned expansion reserves its rail plus approximately 360px panel independently. Matching square bottom-pinned toggles never hide a sidebar, the rejected compressed horizontal room-tab row is removed, View can hide/restore either system without changing the other, and contextual messages still overlay without remounting media
- the Stage layout toolbar and 16:9 output share one center-column width contract as either sidebar pins, collapses, hides, or peeks; shared positive/destructive button states retain readable dark/light contrast, and the Runtime-owned Fill/Fit room setting remains in the right Room panel
- a Studio-local port of Public's footer bar structure, spacing, separators, typography, Runtime/Auth status disclosure, version/build tooltip, dark/light glass treatment, and responsive stacking; it remains a shell grid row and cannot overlap either sidebar toggle, the production dock, or Stage
